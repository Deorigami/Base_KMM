...
abstract class BaseRecyclerViewAdapter<ITEM : Any,V : View>() : ListAdapter<ITEM, BaseRecyclerViewAdapter.ViewHolder<V>>(
    ...
    override fun onBindViewHolder(holder: ViewHolder<V>, position: Int) {
        holder.apply {
            onBind(getItem(absoluteAdapterPosition), absoluteAdapterPosition)
            itemView.setOnClickListener { onItemPressed(absoluteAdapterPosition) }
        }
    }
